...
const props = defineProps({
  ...
  rounded: { type: String, default: 'rounded-[var(--radius-md)]' },
})
...
<template>
  ...
  <div v-else-if="type === 'paragraph'" class="flex flex-col gap-2.5" aria-hidden="true">
    <div
      v-for="i in lines"
      :key="i"
      :class="[BASE, 'rounded-[var(--radius-md)]']"
  ...
  <div v-else-if="type === 'table'" class="flex flex-col gap-2" aria-hidden="true">
    <div
      ...
      <div
        v-for="c in cols"
        :key="c"
        :class="[BASE, 'h-8 rounded-[var(--radius-md)]']"
  ...
  <div v-else-if="type === 'card'"
    class="bg-[var(--surface)] border border-[var(--border-main)] rounded-[var(--radius-xl)] p-5 flex flex-col gap-4"
    aria-hidden="true">
    <div class="flex items-center gap-3">
      <div :class="[BASE, 'w-10 h-10 rounded-[var(--radius-xl)] shrink-0']"></div>
      <div class="flex flex-col gap-2 flex-1">
        <div :class="[BASE, 'h-4 rounded-[var(--radius-md)] w-1/3']"></div>
        <div :class="[BASE, 'h-3 rounded-[var(--radius-md)] w-1/2']"></div>
      </div>
    </div>
    <div class="flex flex-col gap-2">
      <div :class="[BASE, 'h-3 rounded-[var(--radius-md)] w-full']"></div>
      <div :class="[BASE, 'h-3 rounded-[var(--radius-md)] w-5/6']"></div>
      <div :class="[BASE, 'h-3 rounded-[var(--radius-md)] w-3/4']"></div>
    </div>
    <div :class="[BASE, 'h-9 rounded-[var(--radius-md)] w-28']"></div>
  </div>
</template>
